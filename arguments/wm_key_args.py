"""
Watermarking Key Arguments - Extended for LoRA
===============================================
"""
import os. path
...
class WatermarkType(Enum):
    PTW = "ptw"  # original PTW
    LORA = "lora"  # ← 新增：纯 LoRA 方案
...
class WatermarkingKeyArgs:
    ...
    watermark_type: str = field(default=WatermarkType. LORA.value, metadata={
    ...
    decoder_channels: int = field(default=64, metadata={
        "help": "Number of channels in the decoder (for custom decoder architectures)"
    })

    num_blocks: int = field(default=4, metadata={
        "help": "Number of residual blocks in the decoder (for custom decoder architectures)"
    })

    # ═══════════════════════════════════════════════════════════
    # PTW 特有参数（原始 PTW 方案使用，LoRA 方案忽略）
    # ═══════════════════════════════════════════════════════════
